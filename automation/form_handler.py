    ...
    def _smart_update_form(self, page, data):
        ...
        try:
            if isinstance(data, dict):
                ...
                # ✅ Set top-level RBE AFTER SS/Soft Currency (run ONCE to avoid Normal/Hard reset)
                if rbe_target and p_key == "hell":
                    try:
                        rbe_fieldset = page.locator(
                            "fieldset:has(legend:has-text('RBE'))"
                        ).first
                    ...
                    except Exception as _rbe_top_e:
                        print(
                            f"         ⚠️ [Contest Superstar] Set top-level RBE failed (outer try): {_rbe_top_e}"
                        )
